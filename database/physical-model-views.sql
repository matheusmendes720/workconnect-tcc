-- ============================================
-- WorkConnect - Physical Model: VIEWS
-- MySQL Views for Dashboard and Reporting
-- ============================================
-- 
-- Version: 1.0.0
-- Created: 2025-01-12
...
SELECT 
    ...
    ROUND((p.quantidade_atual / NULLIF(p.quantidade_minima, 0)) * 100, 2) AS percentual_minimo,
    p.unidade_medida,
    p.preco_aquisicao,
    p.preco_venda,
    ...
    p.prazo_validade,
    CASE 
        WHEN p.prazo_validade < CURDATE() THEN TRUE
        ELSE FALSE
    END AS vencido,
...
WHERE m.data_hora >= DATE_FORMAT(CURDATE(), '%Y-%m-01')
ORDER BY m.data_hora DESC;

-- View 4: Dashboard de Alertas de Estoque
CREATE OR REPLACE VIEW vw_dashboard_alertas AS
SELECT 
    COUNT(CASE WHEN status = 'CRITICO' THEN 1 END) AS produtos_criticos,
    COUNT(CASE WHEN status = 'BAIXO' THEN 1 END) AS produtos_baixos,
    COUNT(CASE WHEN status = 'OK' THEN 1 END) AS produtos_ok,
...
SELECT 
    ...
    CASE 
        ...
        WHEN e.data_previsao_entrega < CURDATE() THEN 'ATRASADO'
        WHEN e.data_previsao_entrega = CURDATE() THEN 'HOJE'
        ELSE 'EM_DIA'
...
SELECT 
    ...
    (SELECT SUM(valor) FROM transacao_financeira 
     ...
     AND DATE_FORMAT(data_transacao, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')) AS despesas_mes,
