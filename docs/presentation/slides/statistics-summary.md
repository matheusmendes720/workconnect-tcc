...
| **Foreign Keys** | 50+ |
| **Constraints** | 100+ |
| **Extensions** | 2 (pg_trgm, uuid-ossp) |
...
| Type | Count | Purpose |
|------|-------|---------|
| **Primary Keys** | 30+ | Unique identification |
| **Foreign Keys** | 50+ | Relationship integrity |
| **Unique** | 10+ | Uniqueness constraints |
| **B-Tree** | 60+ | Standard lookups |
| **GIN** | 5+ | Full-text search |
| **Composite** | 15+ | Complex queries |

### Key Indexes

**Performance Critical:**
- `idx_produto_codigo` - Product code lookup
- `idx_venda_data` - Sales date queries
- `idx_trans_data` - Transaction date queries
- `idx_mov_data` - Movement date queries

**Full-Text Search:**
- `idx_produto_nome` - Product name search (GIN)
- `idx_cliente_nome` - Customer name search (GIN)
- `idx_fornecedor_nome` - Supplier name search (GIN)
...
| **Foreign Key** | 50+ | Relationship integrity |
| **Unique** | 15+ | Email, CNPJ, codes |
| **Check** | 30+ | Status values, ranges |
...
**Business Rules:**
- Product status calculation rules
- Sale total = subtotal - discount + addition
...
| Type | Usage | Examples |
|------|-------|----------|
| **BIGSERIAL** | Primary keys | All table IDs |
| **VARCHAR** | Text fields | Names, codes, emails |
| **DECIMAL(10,2)** | Money/quantities | Prices, amounts |
| **INTEGER** | Counts | Quantities, counts |
| **BOOLEAN** | Flags | Active, viewed, etc. |
| **TIMESTAMP** | Dates/times | Created, updated |
| **DATE** | Dates | Birth date, expiry |
| **TEXT** | Long text | Descriptions, notes |
| **JSONB** | Flexible data | Permissions, parameters |
...
- **Complex joins**: < 100ms (optimized)
- **Dashboard views**: < 200ms (materialized)
- **Full-text search**: < 50ms (GIN indexes)

### Scalability

- **Tables**: Designed for 100K+ rows
- **Indexes**: Optimized for frequent queries
- **Partitioning**: Ready for large tables (audit)
- **Archiving**: Views support historical data
